...
impl Codec {
    ...
    pub fn index_in_complete_binary_tree(root: &Option<Rc<RefCell<TreeNode>>>) -> Vec<usize> {
        let mut res = vec![];
        fn find(node: &Option<Rc<RefCell<TreeNode>>>, res: &mut Vec<usize>, node_index: usize) {
            if let Some(node) = node {
                println!("node index overflow {}", node_index);
                res.push(node_index);
                if node.borrow().left.is_some() {
                    find(&node.borrow().left, res, 2 * node_index + 1);
                }
                if node.borrow().right.is_some() {
                    find(&node.borrow().right, res, 2 * node_index + 2);
                }
            }
        }
        find(root, &mut res, 0);
        res.sort();
        // println!("如果按完全二叉树 节点的下标是 {:?}", &res);
        res
    }

    pub fn count_node(root: &Option<Rc<RefCell<TreeNode>>>) -> usize {
        let mut counter = 0;

        fn find(root: &Option<Rc<RefCell<TreeNode>>>, counter: &mut usize) {
            match root {
                None => {}
                Some(node) => {
    ...
    fn serialize(&self, root: Option<Rc<RefCell<TreeNode>>>) -> String {
        ...
        let complete_index_list = Self::index_in_complete_binary_tree(&root);

        if complete_index_list.len() == 0 {
            return "[]".to_string();
        }

        let mut node_list: Vec<Option<i32>> =
            vec![None; complete_index_list[complete_index_list.len() - 1] + 1];
        ...
        while !queue.is_empty() {
            ...
            match node {
                ...
                Some(node) => {
                    // 填入预选的位置。
                    node_list[complete_index_list[index]] = Some(node.borrow().val);
    ...
    fn deserialize_lc(&self, data: String) -> Option<Rc<RefCell<TreeNode>>> {
    ...
    fn deserialize(&self, data: String) -> Option<Rc<RefCell<TreeNode>>> {
        ...
        for i in 0..node_list.len() {
            if node_list[i] != "null".to_string() {
                if (2 * i + 1) < node_list.len() && node_list[2 * i + 1] != "null" {
                    tree_node_list[i].borrow_mut().left = Some(tree_node_list[2 * i + 1].clone());
                }
                if (2 * i + 2) < node_list.len() && node_list[2 * i + 2] != "null" {
                    tree_node_list[i].borrow_mut().right = Some(tree_node_list[2 * i + 2].clone());
                }
            }
        }
