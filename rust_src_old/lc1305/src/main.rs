...
impl Solution {
    ...
    pub fn get_all_elements(
        ...
        let mut nodes1 = vec![];
        let mut nodes2 = vec![];
        fn find(node: Option<Rc<RefCell<TreeNode>>>, nodes: &mut Vec<i32>) {
            if let Some(node) = node {
                let node = node.borrow();

                find(node.left.clone(), nodes);

                nodes.push(node.val);

                find(node.right.clone(), nodes);
            }
        }

        find(root1, &mut nodes1);
        find(root2, &mut nodes2);
...
fn main() {
    ...
    let st1 = "[1,null,8]".to_string();
    let root1 = deserialize(st1);

    let st2 = "[1,0,3]".to_string();
    let root2 = deserialize(st2);

    println!("{:?}", Solution::get_all_elements(root1, root2));
}
