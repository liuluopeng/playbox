...
impl Solution {
    ...
    // 层序遍历计数遇到空停止  与总的节点数 比较
    pub fn is_complete_tree(root: Option<Rc<RefCell<TreeNode>>>) -> bool {
        fn get_sum_until_null(root: Option<Rc<RefCell<TreeNode>>>) -> usize {
            let mut sum = 0;
        ...
        let mut all_sum = 0;
        fn get_node_sum(root: Option<Rc<RefCell<TreeNode>>>, all_sum: &mut usize) {
            if let Some(node) = root {
                *all_sum += 1;
                get_node_sum(node.borrow().left.clone(), all_sum);
                get_node_sum(node.borrow().right.clone(), all_sum);
            }
        }

        let sum1= get_sum_until_null(root.clone());
        get_node_sum(root, &mut all_sum);
        
        println!("遇到空停止计数：{}  总个数{}", sum1, all_sum);
...
fn main() {
    ...
    let st = "[1,2,3,4,5,null,7]".to_string();
