...
impl Solution {
    ...
    pub fn _gen(start: i32, end: i32) -> Vec<Option<Rc<RefCell<TreeNode>>>> {
        // if start > end {
        //     return vec![];
        // }
        ...
        for i in start..=end {
            ...
            // 小的 左子树    大的 右子树
            // let left_part: Vec<_> = (start..=i-1).collect();
            // let right_part: Vec<_> = (i+1..=end).collect();

            let left_part_candi = Self::_gen(start, i - 1);
            let right_part_candi = Self::_gen(i + 1, end);

            for left_candi_index in 0..left_part_candi.len() {
                for right_candi_index in 0..right_part_candi.len() {
                    // 生成结点
                    let mut root = Rc::new(RefCell::new(TreeNode::new(i)));

                    root.borrow_mut().left = left_part_candi[left_candi_index].clone();
                    root.borrow_mut().right = right_part_candi[right_candi_index].clone();
                    // println!("新产生的片段 {:?}", root);
...
fn main() {
    let mut n = 3;
    let mut res = Solution::generate_trees(n);
