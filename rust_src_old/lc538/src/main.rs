...
impl Solution {
    pub fn convert_bst(root: Option<Rc<RefCell<TreeNode>>>) -> Option<Rc<RefCell<TreeNode>>> {
        ...
        fn de_in_order(node: Option<Rc<RefCell<TreeNode>>>, sum: &mut i32) {
            if let Some(node) = node {
                let mut node: std::cell::RefMut<'_, TreeNode> = node.borrow_mut();

                de_in_order(node.right.clone(), sum);

                *sum += node.val;
                node.val = *sum;

                // println!("{:?}", node.val);
                de_in_order(node.left.clone(), sum);
            }
        }

        de_in_order(root.clone(), &mut sum);
...
fn main() {
    ...
    let st1 = "[4,1,6,0,2,5,7,null,null,null,3,null,null,null,8]".to_string();
    let root = deserialize(st1);
