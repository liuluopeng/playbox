...
impl Solution {
    pub fn is_valid_bst(root: Option<Rc<RefCell<TreeNode>>>) -> bool {
        fn find(root: &Option<Rc<RefCell<TreeNode>>>, last_val: &mut Option<i32>, res: &mut bool) {
            if *res == false {
                return;
            }
            if let Some(node) = root {
                ...
                find(&node.left, last_val, res);

                // println!("{:?} {:?}", last_val, node.val);
                if let Some(v) = *last_val {
                    if v >= node.val {
                        // println!("前一个{:?} 大于等于 {:?}  不是平衡二叉树 ", v, node.val);
                        *res = false;
                    }
                }
                *last_val = Some(node.val);

                find(&node.right, last_val, res);
            }
        }

        let mut last_val = None;
        let mut res = true;
        find(&root, &mut last_val, &mut res);
        res
    }
}

fn main() {
    let data = "[5,4,6,null,null,3,7]";
