...
impl Solution {
    pub fn reverse_odd_levels(
        ...
        while q.len() != 0 {
            let mut tmp_q: Vec<Option<Rc<RefCell<TreeNode>>>> = vec![];

            if rev {
                for i in 0..q.len() / 2 {
                    let node_1 = q[i].clone();

                    let node_2 = q[q.len() - 1 - i].clone();

                    match (node_1, node_2) {
                        (Some(node1), Some(node2)) => {
                            let mut node1 = node1.borrow_mut();
                            let mut node2 = node2.borrow_mut();
                            let tmp_var = node1.val;
                            node1.val = node2.val;
                            node2.val = tmp_var;
                        }
            ...
            for i in 0..q.len() {
                if let Some(node) = q[i].clone() {
                    let node = node.borrow();
                    if node.left.is_some() {
                        tmp_q.push(node.left.clone());
                    }
                    if node.right.is_some() {
                        tmp_q.push(node.right.clone());
                    }
                }
            }
...
#[cfg(test)]
mod tests {
    use crate::solution::{string2tree, tree2string, Solution};

    #[test]
    fn it_works() {
        let st = "[2,3,5,8,13,21,34]";
        let root = string2tree(st);
        let root_res = Solution::reverse_odd_levels(root);

        println!("{:?} res ", tree2string(root_res));
    }
}
...
fn count_node(root: &Option<Rc<RefCell<TreeNode>>>) -> i32 {
    let mut counter = 0;
