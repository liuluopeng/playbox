...
pub fn tree2string(root: Option<Rc<RefCell<TreeNode>>>) -> String {
    let mut node_list: Vec<Option<i32>> = vec![];
    let mut queue: Vec<Option<Rc<RefCell<TreeNode>>>> = vec![];

    let last_index = count_node(&root); // 最后一个叶子节点的序号
...
pub fn string2tree(data: &str) -> Option<Rc<RefCell<TreeNode>>> {
    ...
    // 创建节点
    for (_index, st) in node_list.iter().enumerate() {
